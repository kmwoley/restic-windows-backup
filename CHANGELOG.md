...
Added support for backing up removable drives (i.e. external USB disks). It's now possible to define a backup source by it's Volume lable, device Serial Number, or the hardware Name. 
...
## Enhancements
- External, removalbe disk drives (i.e. USB hard drives) can be identified by their Volume Label, Serial Number, or Device Name. For example, if you have an external device with the Volume Lable "MY BOOK", you can define a backup source as `$BackupSources["MY BOOK"]`. I would recommend using the device serial number to identify external drives to backup, which you can find using the Powershell `get-disk` command.
